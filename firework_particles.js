...
export class Particle {
    constructor(x, y, dx, dy, size, color, doUpdate = true, fade = true, decay=0.01) {
        this.x = x;
        this.y = y;
        this.dx = dx;
        this.dy = dy;
...
export function initParticles(x, y, count, color = null, effect = "circle") {
    ...
    console.log(`Effect chosen: ${effect}`);
    ...
    switch (effect) {
        ...
        case "down": {
            for (let i = 0; i < count; i++) {
                const angle = Math.sign(Math.random() - 0.5) * (Math.PI * 1.5 + Math.PI * 0.76 * (Math.random() - 0.5));
                const speed = Math.random() * 3 + 1; 
                const dx = Math.cos(angle) * speed;
                const dy = Math.sin(angle) * speed;
                newParticles.push(
                    new Particle(x, y, dx, dy, Math.random() * 7 + 3, getColor(), true, true, 0.003)
                );
            }
            break;
        }
